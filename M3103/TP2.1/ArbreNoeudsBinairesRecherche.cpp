...
int ArbreNoeudBinaireRecherche<TypeInfo>::getHauteurWorker(NoeudBinaire<TypeInfo>* ptrRac) const {
    ...
    if (ptrRac == nullptr) {
        ...
        return max(getHauteurWorker(ptrRac->getPtrFilsGauche()),
                   getHauteurWorker(ptrRac->getPtrFilsDroit())) 
               + 1;
    }    
...
int ArbreNoeudBinaireRecherche<TypeInfo>::getNombreDeNoeudsWorker(NoeudBinaire<TypeInfo>* ptrRac) const {
    ...
    if (ptrRac == nullptr) {
        ...
        return getNombreDeNoeudsWorker(ptrRac->getPtrFilsGauche())
               + getNombreDeNoeudsWorker(ptrRac->getPtrFilsDroit()) 
               + 1;
    }    
...
bool ArbreNoeudBinaireRecherche<TypeInfo>::estInfoPresenteWorker(NoeudBinaire<TypeInfo>* ptrRac,
    ...
    if (ptrRac == nullptr) {
        ...
        if (ptrRac->getInfo() > infoCible) {
            estInfoPresenteWorker(ptrRac->getRefPtrFilsGauche(), infoCible);
        } else {
            estInfoPresenteWorker(ptrRac->getRefPtrFilsDroit(), infoCible);
        }
    }
...
int ArbreNoeudBinaireRecherche<TypeInfo>::getNombreDeFeuillesWorker(const NoeudBinaire<TypeInfo>* ptrRac) const {
    ...
    if (ptrRac == nullptr) {
        return 0;
               ...
               || ptrRac->getPtrFilsDroit() == nullptr) {
        return 1;
    }
    else {
        return getNombreDeFeuillesWorker(ptrRac->getPtrFilsGauche())
               + getNombreDeFeuillesWorker(ptrRac->getPtrFilsDroit());
    }
...
bool ArbreNoeudBinaireRecherche<TypeInfo>::aMemeGeometrieQueWorker(const NoeudBinaire<TypeInfo>* monPtrRac, const NoeudBinaire<TypeInfo>* sonPtrRac) const {
    ...
    if (monPtrRac == nullptr && sonPtrRac == nullptr) {
        ...
        return aMemeGeometrieQueWorker(monPtrRac->getPtrFilsGauche(),
                                       sonPtrRac->getPtrFilsGauche())
               && aMemeGeometrieQueWorker(monPtrRac->getPtrFilsDroit(),
                                          sonPtrRac->getPtrFilsDroit());
    }
...
int ArbreNoeudBinaireRecherche<TypeInfo>::getNombreOccurrencesWorker(const NoeudBinaire<TypeInfo>* ptrRac, const TypeInfo& uneInfo) const {
    ...
    if (ptrRac == nullptr) {
        ...
        if (ptrRac->getInfo() == uneInfo) {
            return getNombreOccurrencesWorker(ptrRac->getPtrFilsGauche(), 
                                              uneInfo)
                   + 1;
        } else if (ptrRac->getInfo() > uneInfo) {
            return getNombreOccurrencesWorker(ptrRac->getPtrFilsGauche(), 
                                              uneInfo);
        } else {
            return getNombreOccurrencesWorker(ptrRac->getPtrFilsDroit(), 
                                              uneInfo);
        }
    }
...
template<class TypeInfo>
void ArbreNoeudBinaireRecherche<TypeInfo>::affichePrettyPrintWorker(const NoeudBinaire<TypeInfo>* ptrRac, const string chaine) const {
    if (ptrRac != nullptr){
        cout << ptrRac->getInfo() << " " << endl;
        if (ptrRac->getPtrFilsGauche()) {
            cout << chaine + "+-";
            if (ptrRac->getPtrFilsDroit()) {
                affichePrettyPrintWorker(ptrRac->getPtrFilsGauche(), chaine + "| ");
            } else {
                affichePrettyPrintWorker(ptrRac->getPtrFilsGauche(), chaine + "  ");
            }
        ...
        if (ptrRac->getPtrFilsDroit()) {
            cout << chaine + "+-";
            affichePrettyPrintWorker(ptrRac->getPtrFilsDroit(), chaine + "  ");
        }
    } 
}

template<class TypeInfo>
void ArbreNoeudBinaireRecherche<TypeInfo>::afficheParcoursLargeurWorker(const NoeudBinaire<TypeInfo>* ptrRac) const {
...
template<class TypeInfo>
void ArbreNoeudBinaireRecherche<TypeInfo>::affichePrettyPrint() const{
    cout << "Affichage pretty print : " << endl;
    affichePrettyPrintWorker(ptrRacine, "");
    cout << endl;
}

template<class TypeInfo>
void ArbreNoeudBinaireRecherche<TypeInfo>::afficheParcoursLargeur() const{
