...
bool estTriee(forward_list<T>& uneListe) {
    auto prec = uneListe.begin();
    for ( auto it = uneListe.begin(); it != uneListe.end(); ++it ) {
        if (*it >= *prec) {
            prec = it;
        } else {
            return false;
...
forward_list<T> nouvelleListeTrieeSansDuplication(forward_list<T>& listeSource) {
    forward_list<T> liste;
    auto prec = listeSource.before_begin();
    for ( auto it = listeSource.begin(); it != listeSource.end(); ++it ) {
        if (*it > *prec) {
            liste.push_front(*it);
            prec = it;
        } 
